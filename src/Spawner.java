import java.awt.Color;
import java.awt.Graphics;

public class Spawner extends MovingObject {
	/**
	 * The radius of the ball.
	 */
	int radius;
	/**
	 * Counts the frames between flash on/off.
	 */
	private int counter;
	/**
	 * The number of frames to wait before toggling the flash.
	 */
	private int flashSpeed;
	/**
	 * Controls the flash - true if the ball is filled in, false if it is an
	 * outline.
	 */
	private boolean filledIn;
	public boolean stopped;
	public boolean enemy = true;
	int drawX = (int) getX() - radius;
	int drawY = (int) getY() - radius;

	/**
	 * Calls the superclass constructor, plus sets radius, and flash parameters.
	 * 
	 * @param x
	 *            The x location.
	 * @param y
	 *            The y location.
	 * @param left
	 *            The left edge.
	 * @param right
	 *            The right edge.
	 * @param top
	 *            The top edge.
	 * @param bottom
	 *            The bottom edge.
	 * @return 
	 */
	public Spawner(double x, double y, int left, int right, int top, int bottom) {
		super(x, y, left + 10, right - 10, top + 10, bottom - 10);
		// numbers above must match the radius
		radius = 5;
		counter = 0;
		flashSpeed = (50);
		filledIn = true;
	}

	/**
	 * Controls the animation parameters. (Called once every time the ball
	 * position is updated.)
	 */
	public void animateOneStep() {
		counter++;
		if (counter == flashSpeed) {
			counter = 0;
			if (filledIn)
				filledIn = false;
			else
				filledIn = true;
			for (int i = 21; i < 25; i++){
				GamePanel.ball.add(new FlashingBall(drawX, drawY, 0, 20, 0, 20)) ;
				GamePanel.ball.get(i).setXSpeed(Math.random() * 16-8);
				GamePanel.ball.get(i).setYSpeed(Math.random() * 16-8);
				GamePanel.ball.get(i).setColor(new Color((int) (Math.random() * 256), (int) (Math
						.random() * 256), (int) (Math.random() * 256)));
				GamePanel.ball.get(i).stopped = false;
			}
		}
	}


	/**
	 * Draws the ball
	 * 
	 * @param g
	 *            The graphics context.
	 */
	public void draw(Graphics g) {


		g.setColor(color);
		g.fillRect(radius, radius, radius, radius);
		if (filledIn){
//			int spawnInterval = 1000;
//			for (int i = 0; i < spawnInterval; i++){
//				if ((i%15)== 0){
//					GamePanel.ball.add(new FlashingBall(drawX, drawY, 0, 15, 0, 15));
//					}
//			}
		}
		

	}
	
	/**
	 * Returns true if the ball is filled in.
	 * 
	 * @return true if the ball is filled in, false otherwise.
	 */
	public boolean isFilled(){
		return filledIn;
	}
}